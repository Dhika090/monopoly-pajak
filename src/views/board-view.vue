  ...
  <section>
    <section class="board-view" v-if="board">
      <div class="board-container">
        ...
        />
        <div class="center">
          <div style="margin-left: 1rem;">jalan: {{ currPLayer.name }}</div>
...
export default {
  ...
  async created() {
    ...
    //   newPosition: 10,
    // })
    const updateMsg = `${this.currPLayer?.name} sedang bermain sekarang`
  ...
  methods: {
    ...
    async payToJail() {
      ...
      this.showUpdate('Anda baru saja keluar dari penjara, lempar dadu!')
      this.closeModal()
    },
    ...
    async swichToNextPlayer() {
      ...
      if (this.currPLayer.isInJail) {
        this.showUpdate(
          `${this.currPLayer.name}, Anda berada di penjara, cobalah untuk mendapatkan dubble atau bayar Rp. 10.000`
        )
    ...
    async throwDice() {
      ...
      if (this.currDice) {
        ...
        if (!isDubble) {
          if (this.currPLayer.isInJail) this.showUpdate('Mungkin lain kali')
          return
        }
      }
      const dice = [
    ...
    checkDubbleCount() {
      if (this.currDice[0] === this.currDice[1]) {
        if (this.doubleCount >= 2) {
          this.showUpdate('3 kali ganda .. Anda akan masuk penjara')
          return true
        }
        this.showUpdate('Dobel ! Main lagi.')
        return false
      }
    },
      ...
      if (isTileFree) {
        switch (currTile.type) {
          ...
          case 'jail':
            ...
            this.showUpdate('kamu masuk penjara..')
            this.goToJail()
            break
          case 'visit':
        ...
        if (isNextPayByDice) {
          ...
          this.showUpdate('Lempar dadu untuk mendapatkan bayaran!')
        } else if (
          currTile.type === 'city' ||
